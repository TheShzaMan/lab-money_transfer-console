...
namespace MoneyTransfer
{
    internal class Person
    {
        ...
        {
            //TODO 4: Use string interpolation in a ConsoleWriteLine to display the user's
            //name and also the amount of money in their wallet (using dot notation). One line.
            
            Console.WriteLine($"{name} has ${wallet.money}");
            //Console.WriteLine($"{name} tiene ${wallet.money}\n");
        }
        ...
        public bool TransferMoney(Person receiver, int amountToTransfer)
        {
            ...
            if (amountToTransfer <= wallet.money)
            {
                ...
                Console.WriteLine($"\n{receiver.name} robs {name} of {amountToTransfer} bucks!    press enter"); Console.ReadLine();
                Console.WriteLine($"\nNow...\n");
                return true;
                //wallet.money -= amountToTransfer;
                //receiver.AcceptMoney(amountToTransfer);
                //Console.WriteLine($"¡{receiver.name} le roba ${amountToTransfer} a {name}!\n" +
                //    $"Ahora...\n");
                //return true;
            }
            else
